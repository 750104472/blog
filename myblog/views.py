...
def login(request):
    if request.method == 'GET':
        ...
        users = Member.objects.filter(name=username,nodeId=password)

        if users:
            u= Member.objects.get(name=username)
            u_dict = model_to_dict(u)
            member = {}
            member['name'] = u_dict['name']
            member['node_id'] = u_dict['nodeId']
            member['avatar_url'] = u_dict['avatar']
            member['type'] = u_dict['type']
            user = member
            return update_user(request,user)
        else:
